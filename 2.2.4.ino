...
void setup() {
    ...
    unsigned long startTime = millis();
    bool triggerApMode = false; // 💡 변경 포인트: NVS 보존형 플래그 적용
    ...
    while (millis() - startTime < 5000) {
        if (touchDetected(TOUCH1_PIN) && touchDetected(TOUCH2_PIN)) {
            ...
            delay(2000); 
            if (touchDetected(TOUCH1_PIN) && touchDetected(TOUCH2_PIN)) {
                triggerApMode = true; // 💡 포맷 명령 대신 강제 AP 셋업 모드 트리거 작동
                break; 
            }
        }
        delay(20);
    ...
    // ❌ 기존에 NVS 전체를 싹 날려버리던 wm.resetSettings() 강제 실행부 완전 제거
    ...
    // 💡 변경 포인트: 새로 개편된 다중 접속 헤더에 강제 모드 유무(triggerApMode)를 전달
...
void loop() {
    ...
    // ===== 네트워크 상태 지속 확인 (실시간 와이파이/MQTT 재연결 보완) =====
    if (WiFi.status() != WL_CONNECTED) {
        // 1. 와이파이 자체가 끊긴 경우
        mqttOfflineEffect = true;
        Serial.println("\n⚠️ 와이파이 끊김 감지! NVS 다중 와이파이 재접속 시도...");
        
        // smartWifi.h의 접속 함수를 다시 호출하여 주변 와이파이를 탐색해 붙입니다.
        // 강제 AP 모드가 아니므로 false를 전달합니다.
        ...
        return; // 와이파이가 붙을 때까지 이번 루프는 종료하고 처음으로 돌아감
    } 
    else if (!client.connected()) {
        // 2. 와이파이는 살아있는데 MQTT만 끊긴 경우
        mqttOfflineEffect = true;
        reconnect();
    } 
    else {
    ...
    if (touch1 && touch2) {
        ...
        if (millis() - resetStartTime > 5000) {
            ...
            WiFiManager wm;
            wm.resetSettings(); // 루프 내 강제 5초 유지는 명확한 공장 초기화로 유지
